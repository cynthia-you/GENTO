...
FX_BOOL CFxKineIF::L0_OnInitEnv(FX_CHAR * env_path,FX_INT32 RobotSerial)
{
	m_InitTag = FX_FALSE;
	if (m_Env.OnInitEnv(env_path,RobotSerial) == FX_FALSE)
	{
		return FX_FALSE;
	}
	{
		...
		Matrix4 tool;
		FX_IdentM44(tool);
		if(m_Env.OnGetArmKinePara(RobotType, DH, PNVA, J67)==FX_FALSE)
		{
			printf("Failed to get arm kinematics parameters from environment\n");
			return FX_FALSE;
		}
		...
		FX_Robot_Init_Lmt( Kine_Serial,  PNVA,  J67);
