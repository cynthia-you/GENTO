...
FX_BOOL CFxPln::L0_OnInitEnv_SingleArm(FX_CHAR *env_path, FX_INT32 RobotSerial)
{
	if (RobotSerial < 0 || RobotSerial > 1)
	{
		return FX_FALSE;
	}
	FX_BOOL result = m_KineIF.L0_OnInitEnv(env_path, RobotSerial);
	return result;
}

FX_BOOL CFxPln::L0_OnInitEnv_DualArm(FX_CHAR *env_path)
{
	FX_BOOL result_left = m_Kine_Left_Arm.L0_OnInitEnv(env_path, 0);
	FX_BOOL result_right = m_Kine_Right_Arm.L0_OnInitEnv(env_path, 1);
