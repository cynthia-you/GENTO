...
public:
	...
	/**
	 * @brief Initialize the kinematics environment for one robot arm.
	 * @param env_path Path to the robot configuration file. Relative paths are supported.
	 * @param RobotSerial Arm selector: 0 for the left/A arm, 1 for the right/B arm.
	 * @return FX_TRUE if the configuration is loaded and the arm is initialized; otherwise FX_FALSE.
	 * @note One CFxKineIF instance should not be shared between different arms after initialization.
	 */
	FX_BOOL L0_OnInitEnv(FX_CHAR *env_path, FX_INT32 RobotSerial);
