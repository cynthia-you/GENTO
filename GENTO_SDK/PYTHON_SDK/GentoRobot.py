...
class HEAD_OUT(Structure):
    """Head feedback data."""
    _fields_ = [("m_HEAD_FBK_Joint_Pos", c_float * 3)]


class HEAD_IN(Structure):
    """Head command data."""
    _fields_ = [("m_HEAD_CMD_Joint_Pos", c_float * 3)]
...
class LIFT_OUT(Structure):
    """Lift feedback data."""
    _fields_ = [("m_LIFT_FBK_Joint_Pos", c_float * 2)]


class LIFT_IN(Structure):
    """Lift command data."""
    _fields_ = [("m_LIFT_CMD_Joint_Pos", c_float * 2)]
...
class HAND_OUT(Structure):
    """Hand feedback data."""
    _fields_ = [
        ("m_HAND_FBK_Joint_Pos", c_int16 * 24),
        ("m_HAND_FBK_Joint_Tor", c_int16 * 24),
        ("m_HAND_FBK_F", c_int16 * 24),
    ]
...
class HAND_IN(Structure):
    """Hand command data."""
    _fields_ = [
        ("m_HAND_CMD_Joint_Pos", c_int16 * 24),
        ("m_HAND_CMD_Joint_Tor", c_int16 * 24),
    ]
...
class HAND_SET(Structure):
    """Hand configuration."""
    _fields_ = [
        ("m_HAND_Ctrl_KP", c_int16 * 24),
        ("m_HAND_Ctrl_KD", c_int16 * 24),
        ("m_HAND_Ctrl_Vel", c_int16 * 24),
    ]
...
class GentoRobot:
    ...
    def _init_funcs(self):
        ...
        d.FX_L1_Terminal_GetData.argtypes = [c_int32, POINTER(c_int32), POINTER(c_ubyte * 64)]
        d.FX_L1_Terminal_GetData.restype = c_int32

        d.FX_L1_Terminal_SetData.argtypes = [c_int32, c_int32, POINTER(c_ubyte), c_uint32]
    ...
    def terminal_get(self, terminal_type: int) -> Tuple[int, bytes]:
        ...
        @param terminal_type: Terminal type (FXTerminalType).
        @return: Tuple (channel_type, data_bytes).
        """
        output_chn = c_int32()
        data = (c_ubyte * 64)()
        with self._lock:
            n = self.dll.FX_L1_Terminal_GetData(terminal_type, byref(output_chn), data)
        if n <= 0:
            return (0, b'')
        return (output_chn.value, bytes(data[:n]))

    def terminal_set(self, terminal_type: int, chn_type: int, data) -> int:
        ...
        with self._lock:
            return self.dll.FX_L1_Terminal_SetData(terminal_type, chn_type, buf, data_len)
    ...
    @staticmethod
    def _get_kinematics_error_msg(error_code: int) -> str:
        errors = {
            0: "Success",
            -1: "Failed",
        }
    ...
    def _kinematics_check(self, ret: int, raise_error=False) -> Union[int, str]:
        func_name = inspect.currentframe().f_back.f_code.co_name
        if ret == 0:
            return 0
        msg = self._get_kinematics_error_msg(ret)
        print(f"[Warning] {func_name} failed: {msg}")
        if raise_error:
            raise RuntimeError(msg)
        return f"{ret}: {msg}"
